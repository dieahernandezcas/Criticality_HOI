...
def timeIntegration(params):
    ...
    # --- NEW: Add beta_val and gamma_val to params with default values ---
    params["beta_val"] = params.get("beta_val", 0.1)
    params["gamma_val"] = params.get("gamma_val", 3.0)
    ...
    a_exc = params["a_exc"]  # This parameter is for the old sigmoid, might become unused
    a_inh = params["a_inh"]  # This parameter is for the old sigmoid, might become unused
    mu_exc = params["mu_exc"]  # This parameter is for the old sigmoid, might become unused
    mu_inh = params["mu_inh"]  # This parameter is for the old sigmoid, might become unused
    ...
    # --- NEW: Get beta_val and gamma_val from params to pass to jitted function ---
    ...
    return timeIntegration_njit_elementwise(
        ...
        tau_exc,
        tau_inh,
        a_exc, # Keep for now, but not used by new S_E/S_I
        a_inh, # Keep for now, but not used by new S_E/S_I
        mu_exc, # Keep for now, but not used by new S_E/S_I
        mu_inh, # Keep for now, but not used by new S_E/S_I
        c_excexc,
        c_excinh,
        c_inhexc,
        c_inhinh,
        ...
        tau_ou,
        sigma_ou,
        beta_val,  # NEW: Pass beta_val
        gamma_val, # NEW: Pass gamma_val
    )
...
def timeIntegration_njit_elementwise(
    ...
    tau_ou,
    sigma_ou,
    beta_val,  # NEW: Add beta_val to function signature
    gamma_val, # NEW: Add gamma_val to function signature
):
    ### integrate ODE system:

    def S_E(x):
        # Apply the activation function only for x > 0, otherwise 0
        x_positive = np.maximum(0, x)
        
        # Calculate the argument for tanh, including the gamma * s^2 term
        tanh_arg = x_positive + gamma_val * (x_positive**2)
        
        # Apply beta * tanh()
        result = beta_val * np.tanh(tanh_arg)
        
        # The line 'result[x <= 0] = 0.0' was removed as np.maximum(0, x) already handles this.
        # This was causing a Numba TypingError for scalar inputs.
